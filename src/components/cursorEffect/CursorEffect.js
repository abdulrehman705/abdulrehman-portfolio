...
function drawStar(ctx, x, y, spikes, outerR, innerR, rotation) {
...
function CursorEffect({ theme }) {
  const canvasRef = useRef(null);
  const overlayCanvasRef = useRef(null);
  const animationFrameId = useRef(null);
  const sparkLoopIdRef = useRef(null);
  const sparksRef = useRef([]);
  const ringsRef = useRef([]);
  const mouseRef = useRef({ x: -100, y: -100 });
  const cursorRef = useRef({ x: -100, y: -100, vx: 0, vy: 0 });

  const themeColors = useRef([
    hexToNormalized(theme.imageHighlight),
    hexToNormalized(theme.jacketColor),
    hexToNormalized(theme.highlight),
    hexToNormalized(theme.splashBg),
    hexToNormalized(theme.secondaryText),
  ]);

  const themeColorsRgb = useRef([
    hexToRgb255(theme.imageHighlight),
    hexToRgb255(theme.jacketColor),
    hexToRgb255(theme.highlight),
    hexToRgb255(theme.splashBg),
    hexToRgb255(theme.secondaryText),
  ]);
  ...
  useEffect(() => {
    ...
    function getWebGLContext(cvs) {
      ...
      let glCtx = cvs.getContext("webgl2", params);
      const isWebGL2 = !!glCtx;
      if (!isWebGL2)
        glCtx =
          cvs.getContext("webgl", params) ||
          cvs.getContext("experimental-webgl", params);
    ...
    function splat(x, y, dx, dy, color, radiusScale) {
      ...
      gl.uniform1f(
        ...
        correctRadius((config.SPLAT_RADIUS / 100.0) * r)
      );
      blit(velocity.write);
      velocity.swap();
    ...
    function generateColor() {
      const tc = themeColors.current;
      const base = tc[colorIndex % tc.length];
      colorIndex++;
      const variation = 0.15;
      return {
        r: Math.max(0, Math.min(1, base.r + (Math.random() - 0.5) * variation)) * 0.18,
        g: Math.max(0, Math.min(1, base.g + (Math.random() - 0.5) * variation)) * 0.18,
        b: Math.max(0, Math.min(1, base.b + (Math.random() - 0.5) * variation)) * 0.18,
      };
    }
    ...
    function handleMouseDown(e) {
      ...
      // Click sparks (circles + spinning stars) for overlay canvas
      const x = e.clientX;
      const y = e.clientY;
      const tc = themeColorsRgb.current;
      for (let i = 0; i < 20; i++) {
      ...
      // Animated expanding rings around click (2 rings, subtle)
      for (let i = 0; i < 4; i++) {
        const delay = i * 3;
  ...
  useEffect(() => {
    ...
    function sparkLoop() {
      ...
      // Animated expanding rings (drawn behind sparks)
      ...
      for (let i = rings.length - 1; i >= 0; i--) {
        ...
        ring.radius += (ring.maxRadius - ring.radius) * 0.065;
        ...
        ctx.strokeStyle = `rgba(${ring.color[0]},${ring.color[1]},${ring.color[2]},${ring.life * 0.5})`;
      ...
      for (let i = sparks.length - 1; i >= 0; i--) {
        ...
        if (s.isStar) {
          ...
          drawStar(ctx, s.x, s.y, 4, s.size * s.life, s.size * s.life * 0.35, s.rotation);
          ctx.fill();
        } else {
      ...
      // Smooth custom cursor (spring-style follow)
      const mouse = mouseRef.current;
      const cursor = cursorRef.current;
      const stiffness = 0.18;
      const damping = 0.75;
      cursor.vx += (mouse.x - cursor.x) * stiffness;
      cursor.vy += (mouse.y - cursor.y) * stiffness;
      cursor.vx *= damping;
      cursor.vy *= damping;
      cursor.x += cursor.vx;
      cursor.y += cursor.vy;
      ...
      sparkLoopIdRef.current = requestAnimationFrame(sparkLoop);
    }
    ...
    return () => {
      ...
      if (sparkLoopIdRef.current) {
        cancelAnimationFrame(sparkLoopIdRef.current);
      }
    };
  }, []);
